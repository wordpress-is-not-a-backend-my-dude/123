...
const TopBar: React.FC = () => {
  ...
  return (
    <>
      <div className={`fixed top-0 left-0 right-0 ${isMobileDevice ? 'h-12' : 'h-6'} bg-gray-800/80 backdrop-blur-md text-white flex items-center justify-between px-2 z-50 text-xs`}>
        <div className="flex items-center space-x-4 overflow-x-auto">
          <button 
            className="hover:bg-white/10 rounded p-1 transition-colors duration-200"
            onClick={toggleVerityOSMenu}
          >
            <img src="https://cloud.artopia.dev/storage/verityicon.png" alt="Verity" width={isMobileDevice ? 20 : 14} height={isMobileDevice ? 20 : 14} />
          </button>
          {!isMobileDevice && menuItems.map((item) => (
            <button
              key={item.name}
        ...
        <div className={`${isMobileDevice ? 'absolute left-12' : 'absolute left-1/2 transform -translate-x-1/2'} flex items-center space-x-2`}>
          {cryptoIcons.map((crypto) => (
            ...
            >
              <img src={crypto.icon} alt={crypto.name} width={isMobileDevice ? 20 : 14} height={isMobileDevice ? 20 : 14} />
            </button>
          ))}
        </div>
        ...
        <button 
          ...
          >
            <img 
              ...
              alt="Wallet" 
              width={isMobileDevice ? 20 : 14} 
              height={isMobileDevice ? 20 : 14} 
            />
          </button>
        </div>
      </div>
