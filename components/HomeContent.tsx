...
const HomeContent: React.FC = () => {
  ...
  useEffect(() => {
    if (stage === 'os' && isMobile()) {
      const timer = setTimeout(() => {
        setShowMobileComingSoon(true);
      }, 15000); // 15 seconds
  ...
  const handleAppSearch = (appName: string) => {
        ...
        <AnimatePresence>
          ...
          {stage === 'os' && (
            ...
            >
              ...
              <MobileComingSoon
                isOpen={showMobileComingSoon}
                onClose={() => setShowMobileComingSoon(false)}
              />
            </motion.div>
          )}
        </AnimatePresence>
