...
const LandingPage: React.FC<LandingPageProps> = ({ onExplore }) => {
  ...
  const validPasscodes = process.env.NEXT_PUBLIC_VALID_PASSCODES?.split(',') || [];
  console.log('Valid Passcodes:', validPasscodes);

  const handleSecretSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    console.log('Entered Passcode:', secretCode);
    console.log('Passcode Match:', validPasscodes.includes(secretCode));
    if (validPasscodes.includes(secretCode)) {
